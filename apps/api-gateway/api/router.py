...
# StreamingDebugger import removed - using simplified debug logging
...
async def process_message_streaming(
    ...
    try:
        ...
        try:
            ...
            # Initialize simple debug tracking
            debug_enabled = os.getenv("DEBUG_STREAMING", "0") == "1"
            chunk_count = 0
            content_chunks = 0
            start_time = time.time()
            if debug_enabled:
                print(f"[{request_id}] 🐛 Starting streaming debug for {mode} mode with {model or 'default'} model")
            ...
            async for chunk in orchestrator.stream_run(mode, message, history):
                ...
                if isinstance(chunk, str):
                    ...
                    # Try other common content attributes
                    else:
                        extracted_content = None
                        # Try common content extraction patterns
                        for attr in ['content', 'text', 'message']:
                            if hasattr(chunk, attr):
                                extracted_content = getattr(chunk, attr)
                        ...
                        if extracted_content:
                            ...
                            if debug_enabled:
                                print(f"[{request_id}] 📤 Extracted content: '{str(extracted_content)[:50]}{'...' if len(str(extracted_content)) > 50 else ''}'")
                        else:
                            # Log empty chunks for debugging and track them
                            empty_chunk_count += 1
                            if debug_enabled:
                                print(f"[{request_id}] ⚠️  Empty chunk #{empty_chunk_count}: type={chunk_type}")
            ...
            # Generate simple debug summary
            if debug_enabled:
                duration = time.time() - start_time
                empty_chunks = chunk_count - content_chunks
                empty_ratio = empty_chunks / chunk_count if chunk_count > 0 else 0
                print(f"[{request_id}] 📊 STREAMING SUMMARY:")
                print(f"[{request_id}]   Duration: {duration:.2f}s")
                print(f"[{request_id}]   Chunks: {chunk_count} (Empty: {empty_chunks}, Content: {content_chunks})")
                print(f"[{request_id}]   Empty Ratio: {empty_ratio:.2f}")
                print(f"[{request_id}]   Content Length: {len(content_buffer)} chars")
