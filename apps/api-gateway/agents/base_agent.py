...
class StreamingMetrics:
    ...
    def log_completion(self, elapsed: float, tokens_generated: int, iterations: int):
        """Log completion metrics for the streaming session"""
        total_time = time.time() - self.start_time
        chunks_per_second = self.chunk_count / total_time if total_time > 0 else 0
        
        print(f"[METRICS] Stream completion:")
        print(f"[METRICS] - Total time: {total_time:.2f}s")
        print(f"[METRICS] - Elapsed: {elapsed:.2f}s") 
        print(f"[METRICS] - Tokens generated: {tokens_generated}")
        print(f"[METRICS] - Iterations: {iterations}")
        print(f"[METRICS] - Total chunks: {self.chunk_count}")
        print(f"[METRICS] - Chunks per second: {chunks_per_second:.2f}")
        print(f"[METRICS] - Tool calls: {self.tool_call_count}")
        print(f"[METRICS] - Errors: {self.error_count}")
