...
if SENTRY_AVAILABLE and os.environ.get("SENTRY_DSN"):
    ...
    print("ℹ️  Sentry not configured")
...
# ================================================================================
# SSE STREAMING ENDPOINTS - Native FastAPI streaming
# ================================================================================

from streaming.sse_handler import StreamingHandler
...
@app.post("/ask/stream")
async def stream_ask(request: Request, chat_request: ChatRequest):
    """Stream chat responses in ask mode using SSE."""
    chat_request.mode = "ask"  # Set mode for ask agent
    return await streaming_handler.stream_response(request, chat_request)

@app.post("/analyst/stream")
async def stream_analyst(request: Request, chat_request: ChatRequest):
    """Stream chat responses in analyst mode using SSE."""
    chat_request.mode = "analyst"  # Set mode for analyst agent
    return await streaming_handler.stream_response(request, chat_request)

@app.post("/stream/chat")
async def stream_chat(request: Request, chat_request: ChatRequest):
    """Generic streaming endpoint that respects the mode in the request."""
    # Mode should be set in the request
    if not hasattr(chat_request, 'mode'):
        chat_request.mode = "ask"  # Default to ask mode
    return await streaming_handler.stream_response(request, chat_request)
